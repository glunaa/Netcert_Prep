...
export default function NetPlusPage() {
          ...
          <thead>
            <tr className="border-b border-border">
              {heads.map((h) => (
                <th
                  key={h}
                  className="text-left px-4 py-3 text-subtle font-medium text-xs uppercase tracking-wider"
                >
                  {h}
                </th>
              ))}
  ...
  const Tr = ({
    ...
    >
      {cells.map((c, i) => (
        <td key={i} className="px-4 py-3 text-sm text-subtle align-top">
          {c}
        </td>
      ))}
    </tr>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-100">
            Net<span className="text-accent">+</span> Study Guide
          </h1>
          <p className="text-subtle text-sm mt-1">
            Complete reference covering all N10-009 exam domains.
          </p>
        </div>
        <Link
          to="/netplus/test"
                ...
                {osiLayers.map((layer, i) => (
                  ...
                  >
                    <td className="px-4 py-3.5">
                      <span
                        className={`font-bold font-mono text-lg ${osiColors[layer.number]}`}
                      >
                        {layer.number}
                      </span>
                    </td>
                    <td className="px-4 py-3.5">
                      <span
                        className={`font-semibold ${osiColors[layer.number]}`}
                      >
                        {layer.name}
                      </span>
                    </td>
                    <td className="px-4 py-3.5">
                      <span className="font-mono text-xs text-subtle bg-surface border border-border px-2 py-0.5 rounded whitespace-nowrap">
                        {layer.pdu}
                      </span>
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="flex flex-wrap gap-1">
                        {layer.protocols.map((p) => (
                          <span
                            key={p}
                            className="text-xs text-slate-400 bg-border/40 px-1.5 py-0.5 rounded font-mono"
                          >
                            {p}
                          </span>
                        ))}
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex flex-wrap gap-1">
                        {layer.devices.map((d) => (
                          <span
                            key={d}
                            className="text-xs text-slate-500 bg-surface px-1.5 py-0.5 rounded"
                          >
                            {d}
                          </span>
                        ))}
                    ...
                    <td className="px-4 py-3.5 text-subtle text-xs leading-relaxed">
                      {layer.function}
                    </td>
                  </tr>
                ))}
                  ...
                  cells={[
                    ...
                    <div className="flex flex-wrap gap-1">
                      {l.protocols.map((p) => (
                        <span
                          key={p}
                          className="text-xs bg-border/40 px-1.5 py-0.5 rounded font-mono text-slate-400"
                        >
                          {p}
                        </span>
                      ))}
          ...
          <div>
            ...
            <div className="card p-4 mb-6">
              ...
              <div className="mt-4 pt-4 border-t border-border">
                <p className="text-xs text-subtle">
                  ...
                  Uses 4-way handshake — FIN → ACK → FIN → ACK. Either side can
                  initiate.
                </p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="card p-4">
                <p className="text-xs text-accent uppercase tracking-wider font-mono mb-2">
                  Flow Control
                </p>
                <p className="text-xs text-subtle">
                  Windowing — receiver advertises receive window (rwnd) size.
                  Sender cannot send more than window allows. Window scales
                  dynamically based on buffer availability.
                </p>
              </div>
              <div className="card p-4">
                <p className="text-xs text-accent uppercase tracking-wider font-mono mb-2">
                ...
                <p className="text-xs text-subtle">
                  ...
                  Recovery. TCP adjusts transmission rate based on network
                  conditions (packet loss, RTT).
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card overflow-hidden">
                ...
                <tbody>
                  {filteredPorts.length === 0 ? (
                    ...
                    filteredPorts.map((entry, i) => (
                      <tr
                        key={entry.port}
                        className={`border-b border-border/40 hover:bg-white/5 transition-colors ${i === filteredPorts.length - 1 ? 'border-b-0' : ''}`}
                      >
            ...
            <div className="px-4 py-2.5 border-t border-border bg-surface/50">
              <p className="text-xs text-subtle">
                {filteredPorts.length} of {portEntries.length} ports shown
              </p>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'subnets' && (
        <div className="space-y-6">
          <div className="card overflow-hidden">
            <div className="px-4 py-3 border-b border-border bg-surface/50">
              <p className="text-xs text-subtle">
                Usable hosts = 2<sup>n</sup> − 2 (subtract network and
                broadcast).
              </p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {subnetCheatSheet.map((row, i) => (
                    <tr
                      key={row.prefix}
                      className={`border-b border-border/40 hover:bg-white/5 transition-colors ${i === subnetCheatSheet.length - 1 ? 'border-b-0' : ''}`}
                    >
                      <td className="px-4 py-3.5">
            ...
            {[
              {
                title: 'Usable Hosts',
                formula: '2ⁿ - 2',
                desc: 'n = host bits. Subtract network (all 0s) and broadcast (all 1s).',
              },
              {
                title: 'Number of Subnets',
                formula: '2ˢ',
                desc: 's = subnet bits borrowed from original host portion.',
              },
              {
                title: 'Block Size',
                formula: '256 - mask octet',
                desc: 'The increment between subnet addresses in the last relevant octet.',
              },
            ].map((item) => (
        ...
        <div className="space-y-8">
          <div>
            ...
            <div className="card p-4 bg-accent/5 border-accent/20 mb-6">
              <p className="text-xs text-subtle">
                <span className="text-accent font-semibold">Full address:</span>{' '}
                <code className="font-mono">
                  2001:0db8:0000:0000:0000:ff00:0042:8329
                </code>
              </p>
              <p className="text-xs text-subtle mt-1">
                <span className="text-accent font-semibold">Compressed:</span>{' '}
                <code className="font-mono">2001:db8::ff00:42:8329</code>
              </p>
            </div>
          </div>
              ...
              {[
                {
                  item: 'No broadcasts',
                  detail:
                    'IPv6 uses multicast instead of broadcast. All-nodes multicast = ff02::1.',
                },
                {
                  item: 'No ARP',
                  detail:
                    'Neighbor Discovery Protocol (NDP) replaces ARP. Uses ICMPv6 messages.',
                },
                {
                  item: 'No DHCP required',
                  detail:
                    'SLAAC (Stateless Address Autoconfiguration) uses router advertisements + EUI-64. DHCPv6 still available.',
                },
                {
                  item: 'IPSec built-in',
                  detail:
                    'IPSec support is mandatory in IPv6 (vs optional in IPv4).',
                },
                {
                  item: 'Larger headers',
                  detail:
                    "40-byte fixed header (simpler than IPv4's variable header). Extension headers handle options.",
                },
                {
                  item: 'EUI-64',
                  detail:
                    'Interface ID derived from 48-bit MAC address. Insert FFFE in middle, flip 7th bit of first octet.',
                },
              ].map((item) => (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {wirelessStandards.map((s, i) => (
                      <tr
                        key={s.standard}
                        className={`border-b border-border/40 last:border-b-0 hover:bg-white/5 ${i === 0 ? '' : ''}`}
                      >
                ...
                <div key={band.label} className="card p-4">
                  ...
                  <div className="mb-2">
                    ...
                    <span className="font-mono text-sm text-slate-200">
                      {Array.isArray(band.nonOverlapping)
                        ? band.nonOverlapping.join(', ')
                        : band.nonOverlapping}
                    </span>
                  </div>
                  <p className="text-xs text-subtle leading-relaxed">
              ...
              {wirelessSecurity.map((s) => (
                <Tr
                  ...
                  cells={[
                    ...
                    <span
                      className={`text-xs font-semibold ${s.status.includes('Deprecated') ? 'text-danger' : s.status.includes('Recommended') ? 'text-success' : 'text-yellow-400'}`}
                    >
                      {s.status}
                    </span>,
        ...
        <div className="space-y-8">
          <div>
            ...
            <div className="card p-4 mb-6">
              ...
              <p className="text-xs text-subtle">
                Trunk links carry multiple VLANs using 802.1Q tagging. Each
                frame is tagged with a 4-byte VLAN tag (including 12-bit VLAN ID
                supporting 4,094 VLANs). Native VLAN frames are NOT tagged —
                both ends must agree on native VLAN or you risk VLAN hopping.
                Inter-VLAN routing requires a Layer 3 device (router or L3
                switch with SVIs).
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="card p-4 bg-accent/5 border-accent/20 mb-6">
              <p className="text-xs text-subtle">
                ...
                Lower AD = preferred route when multiple protocols know the same
                destination. Connected = 0, Static = 1, eBGP = 20, EIGRP = 90,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {natTypes.map((n) => (
                <div key={n.type} className="card p-4">
                  ...
                  <div className="mb-2">
                    <span className="text-xs font-mono bg-border/40 text-slate-400 px-2 py-0.5 rounded">
                      {n.mapping}
                    </span>
                  </div>
      ...
      {activeTab === 'security' && (
        <div className="space-y-8">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
              {filteredAttacks.map((a) => (
                <div
                  key={a.name}
                  className="card p-4 border-danger/10 hover:border-danger/25 transition-colors"
                >
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-slate-200 text-sm">
                      {a.name}
                    </h3>
                    <span className="text-xs text-danger bg-danger/10 border border-danger/20 px-2 py-0.5 rounded">
                      {a.category}
                    </span>
                  </div>
                  <p className="text-xs text-subtle leading-relaxed">
          ...
          <div>
            ...
            <Table
              heads={['Protocol', 'Ports', 'Encryption', 'Modes', 'Notes']}
            >
              {vpnProtocols.map((v) => (
                <Tr
                  key={v.protocol}
      ...
      {activeTab === 'troubleshooting' && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="card p-5">
              <h3 className="font-semibold text-slate-200 text-sm mb-3">
                OSI Troubleshooting Approaches
              </h3>
              <div className="space-y-2">
                {[
                  {
                    approach: 'Bottom-Up',
                    desc: 'Start at Layer 1 (physical). Good for new deployments. Check cables, LEDs, then work up.',
                  },
                  {
                    approach: 'Top-Down',
                    desc: 'Start at Layer 7 (application). Good when app works elsewhere. Check app, then work down.',
                  },
                  {
                    approach: 'Divide and Conquer',
                    desc: 'Start in the middle (Layer 3 — ping). If ping works, go up. If not, go down.',
                  },
                  {
                    approach: 'Follow the Path',
                    desc: 'Trace from source to destination systematically, checking each hop.',
                  },
                ].map((a) => (
                  <div key={a.approach}>
                ...
                {[
                  {
                    issue: 'Duplicate IP',
                    cause:
                      'Two devices with same IP. ARP conflicts. Check with arp -a.',
                  },
                  {
                    issue: 'Broadcast storm',
                    cause:
                      'Switching loop without STP. All ports flood continuously.',
                  },
                  {
                    issue: 'DHCP exhaustion',
                    cause: 'DHCP pool full. Clients get APIPA (169.254.x.x).',
                  },
                  {
                    issue: 'DNS failure',
                    cause:
                      'Name resolution fails but IP ping works. Check DNS server reachability.',
                  },
                  {
                    issue: 'Asymmetric routing',
                    cause:
                      'Packets take different paths in/out. Stateful firewall drops return traffic.',
                  },
                  {
                    issue: 'MTU mismatch',
                    cause:
                      'Fragmentation or drops with large packets. Test with ping -f -l 1472.',
                  },
                ].map((i) => (
                  <div key={i.issue}>
      ...
      {activeTab === 'networktypes' && (
        <div className="space-y-8">
          ...
          <div>
            ...
            <Table
              heads={['Category', 'Type', 'Max Speed', 'Max Distance', 'Notes']}
            >
              {cableTypes.map((c) => (
                <Tr
                  key={c.name}
            ...
            <div className="space-y-2">
              {dnsResolutionSteps.map((s) => (
                <div key={s.step} className="card p-3 flex items-start gap-3">
                  <span className="shrink-0 w-6 h-6 rounded-full bg-accent/20 text-accent font-mono text-xs flex items-center justify-center font-bold">
                    {s.step}
                  </span>
                  <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {quickFacts.map((group) => (
            <div key={group.category} className="card p-4">
              <h3 className="text-xs font-mono text-accent uppercase tracking-wider mb-3 pb-2 border-b border-border">
                {group.category}
              </h3>
              <div className="space-y-2">
      ...
      <div className="mt-10 card-glass p-6 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-xl">
        <div>
          ...
          <p className="text-sm text-subtle">
            30 randomized questions covering all Network+ exam domains.
          </p>
        </div>
        <Link
          to="/netplus/test"
