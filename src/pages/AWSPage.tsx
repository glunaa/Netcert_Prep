...
import {
  ...
  sharedResponsibilityExamples,
  iamBasics,
  iamBestPractices,
  coreServicesOverview,
  s3StorageClassesSimple,
  ec2PricingSimple,
  wellArchPillars,
  billingTools,
  supportPlans,
...
const secCategoryList = [
  ...new Set(securityServicesSimple.map((s) => s.category)),
];
...
export default function AWSPage() {
  ...
  const filtered =
    activeCategory === 'All'
      ? awsServicesSummary
      : awsServicesSummary.filter((s) => s.category === activeCategory);
  const filteredSec =
    secFilter === 'All'
      ? securityServicesSimple
      : securityServicesSimple.filter((s) => s.category === secFilter);
  const detailService = coreServicesOverview.find(
    (s) => s.service === serviceDetail,
  );
  ...
  const Tr = ({ cells }: { cells: React.ReactNode[] }) => (
    <tr className="border-b border-border/40 last:border-b-0 hover:bg-white/5 transition-colors">
      {cells.map((c, i) => (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-100">
            AWS <span className="text-aws">Study Guide</span>
          </h1>
          <p className="text-subtle text-sm mt-1">
            Complete reference covering all CLF-C02 exam domains.
          </p>
        </div>
        <Link
          to="/aws/test"
        ...
        <div>
          ...
          {detailService && (
            <div className="card p-5 mb-6 border-aws/30">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div>
                  <h3 className="font-bold text-aws text-lg">
                ...
                <button
                  ...
                  className="text-subtle hover:text-slate-200 transition-colors text-xs"
                >
                  ✕ close
                </button>
              </div>
              ...
              <div>
                <p className="text-xs text-subtle uppercase tracking-wider font-mono mb-2">
                  Key Facts
                </p>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-150 border ${
                  activeCategory === cat
                    ? 'bg-aws text-void border-aws'
                    : 'bg-surface border-border text-subtle hover:text-slate-200 hover:border-slate-500'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {filtered.map((service) => {
              const hasDetail = coreServicesOverview.some(
              ...
              return (
                <div
                  key={service.name}
                  onClick={() =>
                    hasDetail ? setServiceDetail(service.name) : undefined
                  }
                  className={`card p-4 transition-colors border ${hasDetail ? 'cursor-pointer hover:border-aws/40 hover:bg-white/5' : 'hover:bg-white/5'} ${serviceDetail === service.name ? 'border-aws/50 bg-aws/5' : ''}`}
                  style={{
                    borderColor:
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className={catTagCls[service.category] ?? 'tag'}>
                      {service.category}
                    </span>
                  </div>
                  ...
                  {hasDetail && (
                    <p className="text-xs text-aws/60 mt-2">
                      click for details →
                    </p>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
              {cloudBenefits.map((b) => (
                <div key={b.benefit} className="card p-4">
                  <h3 className="font-semibold text-aws text-sm mb-1">
                    {b.benefit}
                  </h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
              {cloudServiceModels.map((m) => (
                <div key={m.model} className="card p-5">
                  <div className="mb-3">
                    <span className="text-2xl font-bold font-mono text-aws">
                      {m.model}
                    </span>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {m.examples.map((e) => (
                      <span
                        key={e}
                        className="text-xs font-mono text-aws bg-aws/10 border border-aws/20 px-2 py-0.5 rounded"
                      >
                        {e}
                      </span>
                    ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {deploymentModels.map((d) => (
                <div key={d.model} className="card p-5">
                  <h3 className="font-bold text-aws text-base mb-2">
                    {d.model}
                  </h3>
                  <p className="text-xs text-subtle mb-3 leading-relaxed">
                    {d.description}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="card p-5 border-aws/20">
                <p className="font-semibold text-aws text-sm mb-3">
            ...
            <div className="card-glass p-3 rounded-xl mb-6">
              <p className="text-xs text-subtle text-center">
                ...
                The more managed the service (Lambda vs EC2), the more AWS is
                responsible for. AWS = physical security + managed service
                infrastructure. You = your data, access control, and config.
              </p>
            </div>
              ...
              {filteredSec.map((s) => (
                <div key={s.service} className="card p-4">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-danger bg-danger/10 border border-danger/20 px-1.5 py-0.5 rounded">
                      {s.category}
                    </span>
                  </div>
                  <p className="text-xs text-subtle leading-relaxed">
            ...
            {wellArchPillars.map((p) => (
              <div key={p.name} className="card p-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className={`font-semibold text-sm ${p.color}`}>
                    {p.name}
                  </h3>
                </div>
                <p className="text-xs text-subtle mb-3 leading-relaxed">
                ...
                <div>
                  <p className="text-[10px] text-subtle uppercase tracking-wider font-mono mb-2">
                    Design Principles
                  </p>
                  <ul className="space-y-1">
                    {p.designPrinciples.map((d) => (
                      ...
                      >
                        <span
                          className={`w-1 h-1 rounded-full mt-1.5 flex-shrink-0 ${p.color}`}
      ...
      {activeTab === 'billing' && (
        <div className="space-y-8">
              ...
              {[
                {
                  model: 'Pay as you go',
                  icon: '⏱',
                  desc: 'Only pay for what you use, when you use it. No long-term contracts or complex licensing.',
                },
                {
                  model: 'Save when you commit',
                  icon: '📅',
                  desc: 'Save up to 72% by committing to 1 or 3 years of usage (Reserved Instances, Savings Plans).',
                },
                {
                  model: 'Pay less by using more',
                  icon: '📊',
                  desc: 'Volume discounts — the more you use (especially S3, data transfer), the less you pay per unit.',
                },
              ].map((p) => (
          ...
          <div>
            ...
            <Table
              heads={['Class', 'Access Frequency', 'Retrieval', 'Best For']}
            >
              {s3StorageClassesSimple.map((s) => (
                <Tr
                  key={s.name}
                  cells={[
                    <span className="font-semibold text-aws text-xs">
                      {s.name}
                    </span>,
          ...
          <div>
                ...
                <div key={p.plan} className="card p-4 flex flex-col">
                  ...
                  <ul className="space-y-1 mt-auto">
                    {p.features.map((f) => (
                      <li
                        key={f}
                        className="flex items-start gap-1.5 text-xs text-subtle"
                      >
                        <span className="text-success mt-0.5 flex-shrink-0">
                          •
                        </span>
            ...
            <div className="mt-4 card p-4 bg-aws/5 border-aws/20">
              <p className="text-xs text-subtle text-center">
                <span className="text-slate-300 font-semibold">Exam tip:</span>{' '}
                Basic is free. Developer for dev/test. Business for production.
                Enterprise for mission-critical. The more you pay, the faster
                the response and the more dedicated support you get.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {infraFacts.map((f) => (
                <div
                  key={f.label}
                  className="card p-4 text-center border-aws/10"
                >
                  <p className="text-3xl font-bold font-mono text-aws mb-1">
                    {f.value}
                  </p>
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                {
                  factor: 'Compliance & Data Sovereignty',
                  detail:
                    'Some regulations require data to stay in a specific country. Always comply with legal requirements first.',
                },
                {
                  factor: 'Proximity to Users',
                  detail:
                    'Choose a Region closest to your users to minimize latency. Use CloudFront for global distribution.',
                },
                {
                  factor: 'Service Availability',
                  detail:
                    'Not all AWS services are available in every Region. Check if the service you need is available in your preferred Region.',
                },
                {
                  factor: 'Pricing',
                  detail:
                    'Pricing varies by Region. us-east-1 (N. Virginia) is often the cheapest. Factor in data transfer costs.',
                },
              ].map((f) => (
                <div key={f.factor} className="card p-4 bg-surface/50">
                  <h4 className="font-semibold text-aws text-xs mb-1">
                    {f.factor}
                  </h4>
                  <p className="text-xs text-subtle">{f.detail}</p>
                </div>
          ...
          <div className="card p-5">
            ...
            <p className="text-xs text-subtle mb-3">
              ...
              (minimum 2) to achieve high availability. If one AZ has an outage,
              your application continues running in the other AZ.
            </p>
              ...
              {[
                {
                  label: 'Single AZ',
                  risk: 'High',
                  desc: 'One data center fails → entire app down. Never for production.',
                },
                {
                  label: 'Multi-AZ',
                  risk: 'Low',
                  desc: '2+ AZs: if one fails, others handle traffic. Standard HA pattern.',
                },
                {
                  label: 'Multi-Region',
                  risk: 'Very Low',
                  desc: 'Global deployment. Handles entire Region failures. DR & compliance.',
                },
              ].map((a) => (
                <div key={a.label} className="card p-3 bg-surface/50">
                  <h4 className="font-semibold text-slate-200 text-xs mb-1">
                    {a.label}
                  </h4>
              ...
              {clfDomainTips.map((d) => (
                <div key={d.domain} className="card p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="font-mono text-lg font-bold text-aws">
                      {d.weight}
                    </span>
                  </div>
            ...
            <div className="card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left px-4 py-3 text-subtle font-medium text-xs uppercase tracking-wider w-[40%]">
                        Scenario / Keyword
                      </th>
                      <th className="text-left px-4 py-3 text-subtle font-medium text-xs uppercase tracking-wider w-[25%]">
                        Answer
                      </th>
                      <th className="text-left px-4 py-3 text-subtle font-medium text-xs uppercase tracking-wider">
                        Why
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {clfScenarioTips.map((t) => (
                      <tr
                        key={t.trigger}
                        className="border-b border-border/40 last:border-b-0 hover:bg-white/2 transition-colors"
                      >
                        <td className="px-4 py-3 text-xs text-subtle">
                          {t.trigger}
                        </td>
                        <td className="px-4 py-3 font-mono font-semibold text-aws text-xs">
                          {t.answer}
                        </td>
              ...
              {clfServiceComparisons.map((c) => (
                <div key={c.title} className="card p-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
                    <div>
                      <p className="font-mono text-xs font-bold text-aws mb-2">
                        {c.left}
                      </p>
                      <ul className="space-y-1">
                        {c.leftPoints.map((p) => (
                          <li
                            ...
                            className="flex items-start gap-2 text-xs text-subtle"
                          >
                            <span className="text-aws mt-0.5">›</span>
                            {p}
                          </li>
                        ))}
                      </ul>
                    ...
                    <div>
                      <p className="font-mono text-xs font-bold text-cyan-400 mb-2">
                        {c.right}
                      </p>
                      <ul className="space-y-1">
                        {c.rightPoints.map((p) => (
                  ...
                  <div className="bg-aws/10 border border-aws/20 rounded-lg px-3 py-2">
                    <p className="text-xs text-aws font-medium">
                      Exam tip: {c.tip}
                    </p>
                  </div>
                </div>
              ...
              {clfCommonGotchas.map((g) => (
                ...
                >
                  <div className="sm:w-1/2">
                    <p className="text-xs text-subtle mb-1 uppercase tracking-wider font-mono">
                      Common mistake
                    </p>
      ...
      <div className="mt-10 card-glass p-6 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-xl">
        <div>
          ...
          <p className="text-sm text-subtle">
            30 randomized questions covering CLF-C02 exam domains.
          </p>
        </div>
        <Link
          to="/aws/test"
