...
export default function VLSMPage() {
  ...
  function addSubnet() {
    setSubnets((prev) => [
      ...
      {
        id: makeId(),
        label: `Subnet ${String.fromCharCode(65 + prev.length)}`,
        hosts: 10,
      },
    ]);
  }
      ...
      prev.map((s) =>
        s.id !== id
          ...
          : {
              ...s,
              [field]:
                field === 'hosts' ? Math.max(1, parseInt(value) || 1) : value,
            },
      ),
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-slate-100">
          VLSM <span className="text-purple-400">Calculator</span>
        </h1>
        <p className="text-subtle text-sm mt-1">
          Variable Length Subnet Masking — plan your IP addressing scheme
          efficiently.
        </p>
      </div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPrefix(
                        Math.min(
                          30,
                          Math.max(8, parseInt(e.target.value) || 24),
                        ),
                      )
                    }
                ...
                {subnets.map((subnet) => (
                  ...
                  >
                    <input
                      ...
                      value={subnet.label}
                      onChange={(e) =>
                        updateSubnet(subnet.id, 'label', e.target.value)
                      }
            ...
            <div className="card overflow-hidden">
              <div className="px-4 py-3 border-b border-border bg-surface/50 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-200">
                  Results — {results.length} subnets allocated
                </h3>
                <span className="tag bg-purple-500/10 text-purple-400 border border-purple-500/20">
                  {networkIp}/{prefix}
                </span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left px-3 py-3 text-subtle font-medium text-xs uppercase tracking-wider whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {results.map((r, i) => (
                      <tr
                        key={r.label + i}
                        className={`border-b border-border/40 hover:bg-white/5 transition-colors ${i === results.length - 1 ? 'border-b-0' : ''}`}
                      >
                        <td className="px-3 py-3 font-semibold text-slate-200">
                          {r.label}
                        </td>
                        <td className="px-3 py-3 font-mono text-subtle">
                          {r.hosts_requested}
                        </td>
                        ...
                        <td className="px-3 py-3 font-mono text-success text-xs">
                          {r.first_host}
                        </td>
                        <td className="px-3 py-3 font-mono text-success text-xs">
                          {r.last_host}
                        </td>
                        <td className="px-3 py-3 font-mono text-subtle text-xs">
                          {r.broadcast}
                        </td>
                        <td className="px-3 py-3 font-mono font-semibold text-accent">
                          {r.usable_hosts}
                        </td>
                      </tr>
                    ))}
        ...
        <div className="space-y-4">
          <div className="card p-5">
            ...
            <p className="text-xs text-subtle leading-relaxed">
              Variable Length Subnet Masking (VLSM) allows dividing a network
              into subnets of different sizes, each sized to fit the actual
              number of hosts needed. This is more efficient than fixed-size
              subnetting (FLSM).
            </p>
          </div>
          ...
          <div className="card p-5">
            ...
            <p className="text-xs text-subtle">
              Example: 50 hosts → 52 → next power of 2 = 64 → /26 (62 usable
              hosts)
            </p>
          </div>
