...
function optionClass(
  ...
  if (!answered) {
    return `${base} border-border bg-surface hover:bg-white/[0.04] hover:border-slate-500 text-slate-200 cursor-pointer`
  }
  ...
  if (idx === selected) {
    return (
      <span className="w-6 h-6 rounded-full bg-danger/20 flex items-center justify-center flex-shrink-0 mt-0.5">
        <svg className="w-3.5 h-3.5 text-danger" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
...
function ResultsScreen({
  results, score, total, passThreshold, label, code, accentCls, tagCls, onRestart,
}: {
  results: TestResult[]
  score: number
  total: number
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className={`card p-8 text-center border ${passed ? 'border-success/30' : 'border-danger/30'}`}>
        ...
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold ${
          ...
          {passed ? (
            ...
            <>
              ...
              Did not pass — need {passThreshold}% ({Math.ceil((passThreshold / 100) * total)} correct)
            </>
          )}
        </div>
        <div className="mt-6 progress-track h-2.5 max-w-sm mx-auto">
          ...
          <div className="absolute top-0 h-full w-px bg-slate-500/50"
...
function TestScreen(props: TestPageProps) {
  const { accentCls, accentBg, accentText, label, code, passThreshold, examKey, tagCls } = props
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <ResultsScreen
          ...
          accentCls={accentCls}
          tagCls={tagCls}
          onRestart={() => { engine.restart(); setScored(false) }}
        />
      </div>
    )
  }
